...
function server(){
    ...
    spid=$((${words[2]}))
    if echo "${words[1]}" | grep -q pid: ; then
...
function help(){
    ...
    if echo "${help}" | grep -q "${expected_help}" ; then
        echo "Help: Passed!"
    else
        echo "Help: Failed!"
    fi
...
function status(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" ; then
        echo "Status: Passed!"
    else
        echo "Status: Failed!"
    fi
...
function np(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_i.txt tests/test_o.txt ; then
        echo "NOP: Passed!"
    else
        echo "NOP: Failed!"
    fi
...
function bcompress(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_bc.txt tests/test_o.txt ; then
...
function bdecompress(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_bdc.txt tests/test_o.txt ; then
...
function gdecompress(){
    ...
    expected_np="$( echo -e "processing\nconcluded (bytes-input: 98, bytes-output: 98)")"
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_gdc.txt tests/test_o.txt ; then
...
function encrypt(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_e.txt tests/test_o.txt ; then
        echo "ENCRYPT: Passed!"
    else
        echo "ENCRYPT: Failed!"
    fi
...
function decrypt(){
    ...
    if echo "${stat}" | grep -q "${expected_stat}" && cmp --silent -- tests/test_de.txt tests/test_o.txt ; then
        echo "DECRYPT: Passed!"
    else
        echo "DECRYPT: Failed!"
    fi
