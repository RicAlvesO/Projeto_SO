...
void terminarGraciosamente(int signum) {
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    sa.sa_handler = terminarGraciosamente;
    sa.sa_flags = 0;
    ...
    */
    ...
    int fifoAux = open(fifo_geral, O_WRONLY); //
    PACOTE pacote;
    while ((pacote = readPacote(fifo)) != NULL) {
        pid_t pid = getPacotePid(pacote);
        if (isPedidoFinishedPacote(pacote)) {
            waitpid(pid, &status, 0);
            removerPedido(gp, pid);
        } else {
            char* fifoEscrita = getPrivateFifoPath(pid, 0);
            if (isServerStatusRequestPacote(pacote)) {
                ...
                freeServer(&server);
                close(fdEscrita);
            } else if (isSendingPedidoPacote(pacote) && !terminandoGraciosamente) {
                char* fifoLeitura = getPrivateFifoPath(pid, 1);
                int fdLeitura = open(fifoLeitura, O_RDONLY);
                PEDIDO pedido = readPedido(fdLeitura);
                setClienteFifoStr(pedido, fifoEscrita);
                setPedidoFifoGeral(pedido, fifoAux);
        ...
        if (gestorIsEmpty(gp) && terminandoGraciosamente) break;
    }
    printf("saiu\n");
    close(fifo);
